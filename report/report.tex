...
\textbf{X.}
PPO-clip (the most common variant of PPO, also the variant implemented in the Stable Baselines\footnote{\url{https://github.com/DLR-RM/stable-baselines3}} library used for this work) achieves this with clipping in the objective function to remove incentives for the new policy to get far from the old policy.
...
\subsection{Model A}
This model was trained for 25 million timesteps on the first level of the game, ``Yoshi's Island 1''.
\begin{figure}[htbp]
    \centering
    \begin{subfigure}{.5\textwidth}
        \centering
    ...
    \end{subfigure}
    \caption{Results of training a PPO model on the level YoshiIsland1 for 25M timesteps.}
    \label{fig:result1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Results of training a PPO model on the level YoshiIsland2 for 25M timesteps.}
    \label{fig:result2}
\end{figure}
